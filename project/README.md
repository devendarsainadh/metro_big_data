# NATICUSdroid Android Permissions - PySpark ETL, MLlib, and Tableau Exports

Repository for an offline-capable big-data workflow using the NATICUSdroid Android permissions dataset.
...
- Ingestion + validation with required-column checks and data quality profiling.
- PySpark DataFrame ETL with broadcast lookup enrichment.
- Custom domain transformer (`PermissionRiskScoreTransformer`).
- Distributed model training with MLlib algorithms:
  - Logistic Regression
  - Random Forest
  - GBT Classifier
- Hyperparameter tuning via `CrossValidator` + `ParamGridBuilder`.
- Single-node sklearn baselines for comparison.
- Model serialization:
...
Generated in `data/tableau_exports/`:
